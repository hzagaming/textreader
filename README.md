...
TextReader is a Chrome and Microsoft Edge extension that reads selected text, extracted articles, or all readable page text with the browser's local system voices.

This repository currently contains **Phase 1 and Phase 2**. Remote AI voices and streaming audio belong to Phase 3 and are intentionally not included yet.
...
- Multilingual sentence segmentation for English, Chinese, Japanese, and mixed text
...
- Local Web Speech API playback with voice, speed, pitch, and volume controls
...
1. Select a short passage and verify the floating speaker starts local speech.
...
10. Change voice/speed/pitch/volume, close the panel, and confirm the values persist.
...
Phase 2 uses only the browser's local Web Speech API. Page DOM, article text, selections, history, and settings are not uploaded. Reading progress and settings remain in `chrome.storage.local`. Password inputs and TextReader's own injected DOM are excluded from selection handling.
...
- Voice availability and pronunciation vary by operating system.
- AI voices, remote providers, streaming audio, cancellation, and paid-audio caching start in Phase 3.
...
- Phase 4: Voice Studio, presets, timing, and word highlighting
